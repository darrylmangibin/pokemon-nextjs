...
import { PokemonDetails } from "@/resources/pokemon/pokemon.interface";
...
const fetchPokemonByName = async (slug: string): Promise<PokemonDetails> => {
  const res = await axios.get<PokemonDetails>(
    `http://localhost:3000/api/pokemon/${slug}`,
  );

  return res.data;
};

const PokemonDetailsPage: FC<PokemonDetailsPageProps> = async ({ params }) => {
  ...
  return (
    <Box>
      ...
      <Grid container spacing={2}>
        <Grid item md={4}>
          <Paper elevation={8}>
            <Grid container sx={{ minHeight: "650px" }} p={4}>
              ...
              <Grid item xs={12}>
                ...
                <Box display="flex" justifyContent="space-evenly" width="100%">
                  {pokemon.types.map(({ type }) => (
                    <Chip key={type.name} label={type.name.toUpperCase()} />
                  ))}
                </Box>
              </Grid>
              <Grid item xs={12}>
                <List>
                  <ListItem>
                    <ListItemText
                      primary="Height"
                      secondary={`${pokemon.height} ft.`}
                    />
                  </ListItem>
                  <ListItem>
                    <ListItemText
                      primary="Weight"
                      secondary={`${pokemon.weight} lbs.`}
                    />
                  </ListItem>
                </List>
              </Grid>
                ...
                <List>
                  {pokemon.abilities.map(({ ability, is_hidden }) => (
                    <Fragment key={ability.name}>
                      <ListItemText
                        primary={ability.name.toUpperCase()}
                        secondary={`Hidden: ${is_hidden ? "Yes" : "No"}`}
                      />
                    </Fragment>
                  ))}
                </List>
